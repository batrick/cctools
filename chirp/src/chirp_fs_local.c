...
{
	PRELUDE("open(`%s', 0x%" PRIx64 ", 0o%" PRIo64 ")", path, flags, mode);
...
{
	PRELUDE("pread(%d, %p, %zu, %" PRId64 ")", fd, buffer, (size_t)length, offset);
	SETUP_FILE
...
{
	PRELUDE("pwrite(%d, %p, %zu, %" PRId64 ")", fd, buffer, (size_t)length, offset);
	SETUP_FILE
...
{
	PRELUDE("lockf(%d, 0o%o, %" PRId64 ")", fd, cmd, len);
	SETUP_FILE
...
{
	struct stat64 linfo;
	PRELUDE("fchmod(%d, 0o%" PRIo64 ")", fd, mode);
	SETUP_FILE
...
{
	PRELUDE("ftruncate(%d, %" PRId64 ")", fd, length);
	SETUP_FILE
...
{
	PRELUDE("symlink(`%s', `%s')", target, path);
	RESOLVE(target)
	RESOLVE(path)
...
{
	PRELUDE("readlink(`%s', %p, %zu)", path, buf, (size_t)length);
	RESOLVE(path)
...
{
	PRELUDE("mkdir(`%s', 0o%" PRIo64 ")", path, mode);
	RESOLVE(path)
...
{
	PRELUDE("access(`%s', %" PRId64 ")", path, amode);
	RESOLVE(path)
...
{
	PRELUDE("chmod(`%s', 0o%" PRIo64 ")", path, mode);
	struct stat64 linfo;
	RESOLVE(path)
	mode &= S_IXUSR|S_IRWXG|S_IRWXO; /* users can only set owner execute and group/other bits */
...
{
	PRELUDE("truncate(`%s', 0d%" PRId64 ")", path, length);
	RESOLVE(path)
...
{
	PRELUDE("utime(`%s', actime = %" PRId64 " modtime = %" PRId64 ")", path, actime, modtime);
	struct utimbuf ut;
	RESOLVE(path)
	ut.actime = actime;
	ut.modtime = modtime;
...
{
	PRELUDE("getxattr(`%s', `%s', %p, %zu)", path, name, data, size);
	RESOLVE(path)
...
{
	PRELUDE("fgetxattr(%d, `%s', %p, %zu)", fd, name, data, size);
	SETUP_FILE
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("lgetxattr(`%s', `%s', %p, %zu)", path, name, data, size);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("listxattr(`%s', %p, %zu)", path, list, size);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("flistxattr(%d, %p, %zu)", fd, list, size);
	SETUP_FILE
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("llistxattr(`%s', %p, %zu)", path, list, size);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("setxattr(`%s', `%s', %p, %zu, %d)", path, name, data, size, flags);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("fsetxattr(%d, `%s', %p, %zu, %d)", fd, name, data, size, flags);
	SETUP_FILE
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("lsetxattr(`%s', `%s', %p, %zu, %d)", path, name, data, size, flags);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("removexattr(`%s', `%s')", path, name);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("fremovexattr(%d, `%s')", fd, name);
	SETUP_FILE
#ifdef CCTOOLS_OPSYS_DARWIN
...
{
	PRELUDE("lremovexattr(`%s', `%s')", path, name);
	RESOLVE(path)
#ifdef CCTOOLS_OPSYS_DARWIN

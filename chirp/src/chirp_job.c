...
int chirp_job_create (chirp_jobid_t *id, json_value *J, const char *subject)
{
	static const char Create[] =
		...
		"INSERT OR ROLLBACK INTO Job (subject, executable, url) VALUES ( ?, ?, ? );"
...
restart:
	...
	sqlcatch(sqlite3_prepare_v2(db, current, -1, &stmt, &current));
	{
		...
		sqlcatch(sqlite3_bind_text(stmt, 2, executable, -1, SQLITE_STATIC));
		sqlcatch(sqlite3_bind_text(stmt, 3, chirp_url, -1, SQLITE_STATIC));
		sqlcatchcode(sqlite3_step(stmt), SQLITE_DONE);

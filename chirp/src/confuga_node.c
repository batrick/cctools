...
CONFUGA_IAPI int confugaS_catalog (confuga *C, const char *catalog)
{
	...
	if (catalog) {
		char *port;
		strncpy(C->catalog_host, "", sizeof(C->catalog_host)); /* zero C->catalog_host */
		strncpy(C->catalog_host, catalog, sizeof(C->catalog_host)-1);
		...
		strncpy(C->catalog_host, CATALOG_HOST_DEFAULT, sizeof(C->catalog_host)-1);
		C->catalog_port = CATALOG_PORT_DEFAULT;
	}
...
static int sn_init (confuga *C, const struct confuga_host *host)
{
	...
	//confugaR_register(C, empty, 0, host); /* FIXME CATCH */
...
CONFUGA_IAPI int confugaS_setup (confuga *C)
{
	static const char SQL[] =
		...
		"	SELECT hostport, root FROM Confuga.StorageNodeActive;"

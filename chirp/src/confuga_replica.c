...
replicated:
	...
	if (fsid) {
		...
		sqlcatch(sqlite3_prepare_v2(db, current, -1, &stmt, &current));
		debug(D_DEBUG, CONFUGA_FID_DEBFMT " from " CONFUGA_SID_DEBFMT " to " CONFUGA_SID_DEBFMT " size=%zu", CONFUGA_FID_PRIARGS(fid), fsid, sid, size);
		sqlcatch(sqlite3_bind_blob(stmt, 1, fid.id, sizeof(fid.id), SQLITE_STATIC));
...
static int schedule_replication (confuga *C)
{
	...
	static const char SQL[] =
		...
		"		)"
		"	SELECT 'NEW', DegradedFile.fid, SourceStorageNode.sid, TargetStorageNode.sid, '(replication)'"
		"		FROM"
		"			DegradedFile"
		...
		"INSERT INTO Confuga.TransferJob (state, fid, fsid, tsid, tag)"
...
static int create (confuga *C, chirp_jobid_t id, const char *fhostport, const char *ffile, const char *fticket, const char *thostport, const char *topen, const char *tag)
{
	static const char SQL[] =
		"UPDATE Confuga.TransferJob"
		"	SET"
...
static int transfer_create (confuga *C)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		const char *tag = (const char *)sqlite3_column_text(stmt, 6);

		CATCHJOB(create(C, id, fhostport, ffile, fticket, thostport, topen, tag));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);

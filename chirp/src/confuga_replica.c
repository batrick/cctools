...
CONFUGA_IAPI int confugaR_replicate (confuga *C, confuga_fid_t fid, sqlite3_int64 sid, time_t stoptime)
{
	static const char SQL[] =
		...
		"SELECT FileReplicas.size, StorageNodeActive.hostport, StorageNodeActive.root"
		"	FROM"
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		snprintf(host_from.hostport, sizeof(host_from.hostport), "%s", (const char *)sqlite3_column_text(stmt, 1));
		snprintf(host_from.root, sizeof(host_from.root), "%s", (const char *)sqlite3_column_text(stmt, 2));
		...
		if (result >= 0) {
			rc = chirp_reli_rename(host_to.hostport, replica_open, replica_closed, time(0)+30);
			if (rc == 0)
				goto replicated;
		} else {
...
CONFUGA_API int confuga_file_create (confuga *C, confuga_file **filep, time_t stoptime)
{
	static const char SQL[] =
		...
		"CREATE TEMPORARY TABLE ConfugaResults AS" /* so we don't hold read locks */
		"	WITH Targets AS ("
		"		SELECT StorageNodeActive.*, SUM(FileReplicas.size) as _bytes"
		"			FROM"
		...
		"	SELECT hostport, root, PRINTF('%s/open/%s', root, UPPER(HEX(RANDOMBLOB(16))))"
		"		FROM Targets"
		"		ORDER BY _bytes ASC, avail DESC;" /* round robin by bytes */
		"SELECT * FROM ConfugaResults;";
...
CONFUGA_API int confuga_file_close (confuga_file *file, confuga_fid_t *fid, confuga_off_t *size, time_t stoptime)
{
	...
	*size = file->size;
	...
	rc = chirp_reli_rename(file->host.hostport, file->path, replica, stoptime); /* FIXME this always deletes new */
	if (rc == 0 || (rc == -1 && errno == EEXIST)) {
		debug(D_DEBUG, "replica %s", rc == -1 ? "already exists" : "created");
...
static int schedule_replication (confuga *C)
{
	...
	static const char SQL[] =
		 ...
		 */
		"DROP VIEW IF EXISTS ConfugaTransferSchedule;"
		"CREATE TEMPORARY VIEW ConfugaTransferSchedule AS"
		"	WITH"
				/* This contains all the Replica of a File AND ongoing transfers of the File to some StorageNode */
		"		Replicas AS ("
		"				SELECT FileReplicas.id AS fid, FileReplicas.size, FileReplicas.minimum_replicas, FileReplicas.sid"
		"					FROM Confuga.FileReplicas"
		"			UNION"
		"				SELECT File.id AS fid, File.size, File.minimum_replicas, TransferJob.tsid AS sid"
		"					FROM Confuga.File JOIN Confuga.TransferJob ON File.id = TransferJob.fid"
		"					WHERE TransferJob.state != 'ERRORED' AND TransferJob.state != 'COMPLETED'"
		"		),"
				/* These are File which are degraded, insufficient replicas exist. */
		"		DegradedFile AS ("
		"			SELECT fid, size, minimum_replicas"
		"				FROM Replicas"
		"				GROUP BY fid"
		"				HAVING COUNT(sid) < minimum_replicas"
		"				ORDER BY minimum_replicas"
		"		),"
		...
		"			SELECT DegradedFile.fid, StorageNodeActive.id as sid"
		"				FROM"
		"					DegradedFile"
		"					JOIN Confuga.Replica ON DegradedFile.fid = Replica.fid"
		"					JOIN Confuga.StorageNodeActive ON Replica.sid = StorageNodeActive.id"
		"				WHERE StorageNodeActive.id NOT IN (SELECT fsid FROM Confuga.TransferJob WHERE state != 'ERRORED' AND state != 'COMPLETED')"
		"		),"
				/* This a StorageNode that can accept a new Replica. It must have sufficient space. */
		"		TargetStorageNode AS ("
		"			SELECT DegradedFile.fid, StorageNodeActive.id as sid, StorageNodeActive.avail"
		"				FROM DegradedFile CROSS JOIN Confuga.StorageNodeActive"
		"				WHERE"
		"					StorageNodeActive.id NOT IN (SELECT sid FROM Replicas WHERE fid = DegradedFile.fid)"
		"					AND StorageNodeActive.avail > DegradedFile.size"
		"				ORDER BY StorageNodeActive.avail"
		"		)"
		"	SELECT 'NEW', DegradedFile.fid, SourceStorageNode.sid, TargetStorageNode.sid"
		"		FROM"
		"			DegradedFile"
		"			JOIN SourceStorageNode ON DegradedFile.fid = SourceStorageNode.fid"
		"			JOIN TargetStorageNode ON DegradedFile.fid = TargetStorageNode.fid"
				/* TODO: Unfortunately this SELECT will pick an arbitrary row
				 * from the GROUP. We would like to prefer TargetStorageNode
				 * with the most space. Fixing this is difficult so arbitary is
				 * fine for now. */
		"		GROUP BY DegradedFile.fid"
				/* Create a new Replica for a File which has a low minimum first. */
		"		ORDER BY DegradedFile.minimum_replicas"
		"		LIMIT 1;"
		"SELECT COUNT(*) FROM ConfugaTransferSchedule;"
		"INSERT INTO Confuga.TransferJob (state, fid, fsid, tsid)"
...
/* FIXME we still need a status and an internal state (I think). The status allows us to have an "intent"... */
...
static int create (confuga *C, chirp_jobid_t id, const char *fhostport, const char *ffile, const char *fticket, const char *thostport, const char *topen)
{
	static const char SQL[] =
		"UPDATE Confuga.TransferJob"
		"	SET"
	...
	//CATCHUNIX(buffer_putliteral(&B, ",\"environment\":{\"CHIRP_CLIENT_TICKETS\":\"ticket\"}")); FIXME
	...
	CATCHUNIX(buffer_putliteral(&B, ",{\"task_path\":\"ticket\",\"serv_path\":\""));
		jsonA_escapestring(&B, fticket);
		CATCHUNIX(buffer_putliteral(&B, "\",\"type\":\"INPUT\",\"binding\":\"LINK\"}"));
	CATCHUNIX(buffer_putliteral(&B, ",{\"task_path\":\"debug\",\"serv_path\":\"/debug.%j\",\"type\":\"OUTPUT\",\"binding\":\"LINK\"}"));
	CATCHUNIX(buffer_putliteral(&B, "]"));
...
/* FIXME: Failure handling. Killable transfers. */
static int transfer_create (confuga *C)
{
	static const char SQL[] =
		...
		"	WHERE state = 'NEW'" /* TODO: AND TransferJob.last_attempt + TransferJob.attempts^2 < strftime('%s', 'now') */
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		const char *thostport = (const char *)sqlite3_column_text(stmt, 4);
		const char *topen = (const char *)sqlite3_column_text(stmt, 5);

		CATCHJOB(create(C, id, fhostport, ffile, fticket, thostport, topen));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
static int commit (confuga *C, chirp_jobid_t id, const char *hostport, chirp_jobid_t cid)
{
	...
	debug(D_DEBUG, "transfer job %" PRICHIRP_JOBID_T ": committing job", id);
...
{
	static const char SQL[] =
		"SELECT TransferJob.id, StorageNode.hostport, TransferJob.cid"
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		CATCHJOB(commit(C, id, hostport, cid));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
static int wait (confuga *C, chirp_jobid_t id, const char *fhostport, chirp_jobid_t cid, const char *thostport, const char *topen)
{
	static const char SQL[] =
		"UPDATE Confuga.TransferJob"
		"	SET progress = ?"
	...
	sqlcatch(sqlite3_prepare_v2(db, current, -1, &stmt, &current));
	CATCHUNIXIGNORE(chirp_reli_stat(thostport, topen, &info, STOPTIME), ENOENT);
	if (rc == 0) {
	...
	debug(D_DEBUG, "transfer job %" PRICHIRP_JOBID_T ": waiting for job", id);
	CATCHUNIX(chirp_reli_job_wait(fhostport, cid, 0, &status, STOPTIME));
	assert(status);
	...
	for (i = 0; i < J->u.array.length; i++) {
		...
		if (jsonA_getname(job, "id", json_integer)->u.integer == cid) {
			...
			sqlcatch(sqlite3_prepare_v2(db, current, -1, &stmt, &current));
			if (error)
				sqlcatch(sqlite3_bind_text(stmt, 1, error->u.string.ptr, -1, SQLITE_STATIC));
			if (exit_code)
				sqlcatch(sqlite3_bind_int64(stmt, 2, exit_code->u.integer));
			if (exit_signal)
				sqlcatch(sqlite3_bind_int64(stmt, 3, exit_signal->u.integer));
			if (exit_status)
				sqlcatch(sqlite3_bind_text(stmt, 4, exit_status->u.string.ptr, -1, SQLITE_STATIC));
			if (status)
				sqlcatch(sqlite3_bind_text(stmt, 5, status->u.string.ptr, -1, SQLITE_STATIC));
			sqlcatch(sqlite3_bind_int64(stmt, 6, id));
			sqlcatchcode(sqlite3_step(stmt), SQLITE_DONE);
			sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
		}
	}
...
{
	static const char SQL[] =
		"SELECT TransferJob.id, fsn.hostport, TransferJob.cid, tsn.hostport, TransferJob.open"
		"	FROM"
		"		Confuga.TransferJob"
		"		JOIN Confuga.StorageNode AS fsn ON TransferJob.fsid = fsn.id"
		"		JOIN Confuga.StorageNode AS tsn ON TransferJob.tsid = tsn.id"
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		const char *fhostport = (const char *)sqlite3_column_text(stmt, 1);
		chirp_jobid_t cid = sqlite3_column_int64(stmt, 2);
		const char *thostport = (const char *)sqlite3_column_text(stmt, 3);
		const char *topen = (const char *)sqlite3_column_text(stmt, 4);
		CATCHJOB(wait(C, id, fhostport, cid, thostport, topen));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
static int reap (confuga *C, chirp_jobid_t id, const char *hostport, chirp_jobid_t cid)
{
	...
	debug(D_DEBUG, "transfer job %" PRICHIRP_JOBID_T ": reaping job", id);
...
static int transfer_reap (confuga *C)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		const char *hostport = (const char *)sqlite3_column_text(stmt, 1);
		chirp_jobid_t cid = sqlite3_column_int64(stmt, 2);
		CATCHJOB(reap(C, id, hostport, cid));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
static int complete (confuga *C, chirp_jobid_t id, const char *hostport, const char *root, const char *open, confuga_fid_t fid)
{
	static const char SQL[] =
		"BEGIN TRANSACTION;"
		"INSERT OR IGNORE INTO Confuga.Replica (fid, sid)"
	...
	sqlite3_stmt *stmt = NULL;
	const char *current = SQL;
	BUFFER_STACK_PRINT(Brepl, CHIRP_PATH_MAX, "%s/file/" CONFUGA_FID_PRIFMT, root, CONFUGA_FID_PRIARGS(fid));

	debug(D_DEBUG, "transfer job %" PRICHIRP_JOBID_T ": completing", id);

	rc = chirp_reli_rename(hostport, open, buffer_tostring(&Brepl), STOPTIME);
	if (rc == -1 && errno == ENOENT) {
		/* previous rename succeeded? but we were not able to update the SQL db... */
		if (chirp_reli_access(hostport, buffer_tostring(&Brepl), R_OK, STOPTIME) == -1)
...
static int transfer_complete (confuga *C)
{
	static const char SQL[] =
		...
		"SELECT TransferJob.id, StorageNode.hostport, StorageNode.root, TransferJob.open, TransferJob.fid"
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		const char *open = (const char *)sqlite3_column_text(stmt, 3);
		confuga_fid_t fid;
		assert(sqlite3_column_type(stmt, 4) == SQLITE_BLOB && (size_t)sqlite3_column_bytes(stmt, 4) == sizeof(fid.id));
		memcpy(fid.id, sqlite3_column_blob(stmt, 4), sizeof(fid.id));
		CATCHJOB(complete(C, id, hostport, root, open, fid));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
CONFUGA_IAPI int confugaR_manager (confuga *C)
{
	...
	CATCH(schedule_replication(C));

	CATCH(transfer_create(C));
	CATCH(transfer_commit(C));
	CATCH(transfer_wait(C));
	CATCH(transfer_reap(C));
	CATCH(transfer_complete(C));

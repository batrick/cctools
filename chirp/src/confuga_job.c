...
// FIXME: batch job operations. create/commit/wait/reap; order by storage node id.
// ????? Actually, batching operations is not really necessary because currently we only have 1 job at a StorageNode...
//       HOWEVER, doing this for transfer jobs could be very useful.
// Need to batch success/errors, destroying the entire operation is not acceptable
// --> generic function that takes callback for each json_value * in array for inputs. output results generically stored in json array returned

// FIXME: there should be retries for some operations like create/commit NOT reschedule
// FIXME: hash for health
...
CONFUGA_API int confuga_job_dbinit (confuga *C, sqlite3 *db)
{
	static const char Initialize[] =
		...
		"INSERT INTO ConfugaJobState (state, running) VALUES"
		"	('NEW', 0),"
		"	('BOUND_INPUTS', 0),"
		"	('SCHEDULED', 1),"
		"	('REPLICATED', 1),"
		...
		"	task_path TEXT NOT NULL,"
		"	PRIMARY KEY (jid, task_path));"
		"CREATE VIEW ConfugaJobRunning AS"
		"	SELECT ConfugaJob.* FROM ConfugaJob NATURAL JOIN ConfugaJobState WHERE ConfugaJobState.running = 1;"
		"CREATE TABLE ConfugaJobTransferJob ("
		"	jid INTEGER NOT NULL REFERENCES ConfugaJob (id),"
		"	tid INTEGER NOT NULL," /* REFERENCES Confuga.TransferJob (id) */
		"	PRIMARY KEY (jid, tid));"
...
static int reschedule (confuga *C, chirp_jobid_t id, int reason)
{
	static const char SQL[] =
		"BEGIN TRANSACTION;"
		"DELETE FROM ConfugaJobTransferJob"
		"	WHERE jid = ?;"
...
static void CATCHJOB (confuga *C, chirp_jobid_t id, int rc)
{
	switch (rc) {
		case 0: /* no error */
			break;
#ifdef ECONNRESET
		case ECONNRESET: /* Transient error. */
#endif
#ifdef ETIMEDOUT
		case ETIMEDOUT: /* Transient error. */
#endif
...
{
	static const char SQL[] =
		"INSERT INTO ConfugaJob (id, state, time_new)"
		"	SELECT Job.id, 'NEW', (strftime('%s', 'now'))"
...
static int dispatch (confuga *C, chirp_jobid_t id)
{
	...
	static const char SQL[] =
		...
		"				FROM Confuga.StorageNodeActive LEFT OUTER JOIN ConfugaJobRunning ON StorageNodeActive.id = ConfugaJobRunning.sid"
		"				GROUP BY StorageNodeActive.id"
		"				HAVING COUNT(ConfugaJobRunning.id) < 1" /* TODO: allow more than one job on a SN */
		"	),"
...
static int job_schedule_fifo (confuga *C)
{
	static const char SQL[] =
		...
		"	LIMIT (CASE WHEN (SELECT COUNT(*) FROM ScheduledJob) < ?1 THEN 1 ELSE 0 END);";
...
static int replicate_push_synchronous (confuga *C, chirp_jobid_t id, sqlite3_int64 sid)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		CATCH(confugaR_replicate(C, fid, sid, STOPTIME));
	}
	sqlcatchcode(rc, SQLITE_DONE);
	sqlcatch(sqlite3_finalize(stmt); stmt = NULL);
...
static int replicate_push_asynchronous (confuga *C, chirp_jobid_t id)
{
	...
	static const char SQL[] =
		...
		"		)"
		"	SELECT 'NEW', NeededFiles.fid, RandomSourceStorageNode.sid, ConfugaJob.sid"
		"		FROM"
		"			ConfugaJob"
...
{
	static const char SQL[] =
		"SELECT executable FROM Job WHERE id = ?;"
...
static int job_create (confuga *C)
{
	static const char SQL[] =
		...
		"	LIMIT (CASE WHEN ?1 == 0 THEN -1 ELSE MAX(0, (?1 - (SELECT COUNT(*) FROM ConfugaJobRunning))) END);";
		...
		if (jsonA_getname(job, "id", json_integer)->u.integer == cid) {
			...
			if (status && strcmp(status->u.string.ptr, "FINISHED") == 0 && exit_status && strcmp(exit_status->u.string.ptr, "EXITED") == 0) {
				...
				if (files) {
					...
					debug(D_DEBUG, "job %" PRICHIRP_JOBID_T ": binding output files", id);
						...
						if (jistype(file, json_object)) {
							...
							if (task_path && serv_path && type) {
								if (strcmp(type->u.string.ptr, "OUTPUT") == 0 && size) {
									...
									debug(D_DEBUG, "job %" PRICHIRP_JOBID_T ": updating ConfugaOutputFile fid = " CONFUGA_FID_PRIFMT " size = %" PRICONFUGA_OFF_T " task_path = `%s'", id, CONFUGA_FID_PRIARGS(fid), (confuga_off_t)size->u.integer, task_path->u.string.ptr);
									sqlcatch(sqlite3_reset(stmt));
									sqlcatch(sqlite3_bind_int64(stmt, 1, id));
...
static int job_complete (confuga *C)
{
	static const char SQL[] =
		...
		"	FROM ConfugaJob NATURAL INNER JOIN ConfugaJobWaitResult"
		"	WHERE state = 'REAPED'"
